...
│   └── test_steganography.cpp  ← Тесты LSB, CRC32, контейнера, метрик
│
├── docs/                       ← Документация
│   ├── architecture.md         ← Архитектура приложения
│   └── algorithms.md           ← Описание алгоритмов стеганографии
│
├── resources/                  ← Ресурсы (тестовые BMP, TXT)
│
├── build_scripts/
│   ├── build_windows.bat       ← Скрипт сборки для Windows (MSVC)
│   ├── build_mingw.bat         ← Скрипт сборки для Windows (MinGW)
...
Сборка и запуск тестов (требует Qt6 с модулем Test):

```bash
cd build
cmake .. -DCMAKE_PREFIX_PATH="C:/Qt/6.11.0/mingw_64"
cmake --build . --target StegoLab_tests
ctest --output-on-failure
```

Тесты покрывают:
- **LSB roundtrip**: внедрение → извлечение для LSB-1/2/4, короткие и длинные сообщения, UTF-8
- **CRC32 integrity**: модификация 1 бита после внедрения → детекция повреждения
- **Контейнерный метод**: внедрение → извлечение архива
- **getCapacity**: корректность вычисления ёмкости
- **calcMetrics**: корректность метрик (MSE, PSNR)

---

## 🔒 Контроль целостности (CRC32)

При внедрении сообщения добавляется 32-битная контрольная сумма CRC32,
вычисленная по (длина сообщения + данные сообщения).

При извлечении CRC32 пересчитывается и сравнивается.
Если хотя бы 1 бит изображения был изменён после внедрения:

- ❌ CRC32 не совпадает
- ⚠️ Пользователь получает предупреждение "Нарушение целостности"
- ✅ Данные всё равно отображаются (для анализа характера повреждения)
